...
export function NextWeekColumn({ dateRange, onGoNext }: NextWeekColumnProps) {
  ...
  return (
    ...
    >
      <p style={{ fontWeight: 700, fontSize: 16, color: 'var(--text-1)', fontFamily: 'var(--font-heading)', marginBottom: 6 }}>
        {t('cal.nextWeek')}
      </p>
      <p style={{ fontWeight: 400, fontSize: 12, color: 'var(--text-3)', fontFamily: 'var(--font-body)' }}>
        {dateRange}
      </p>
      <Sparkles size={20} color="var(--border)" strokeWidth={1.5} style={{ marginTop: 10 }} />
      <div style={{ marginTop: 8, display:'flex', alignItems:'center', gap:3, color:'var(--text-3)', fontSize:11, fontFamily:'var(--font-body)' }}>
        {t('cal.goNext')} <ChevronRight size={12} />
      </div>
    </motion.button>
  )
}
