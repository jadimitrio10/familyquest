...
// Empty init — rewards belong to real members
...
export function RewardsView() {
  ...
  const effectiveNewMember = newMember || members[0]?.id || ''

  const claim = (r: Reward) => {
    const bal = points[r.memberId] ?? 0
    if (bal < r.points || r.claimed) return
    setPoints(p => ({ ...p, [r.memberId]: (p[r.memberId] ?? 0) - r.points }))
  ...
  const add = () => {
    ...
    setRewards(rs => [...rs, { id:`r-${Date.now()}`, title:newTitle.trim(), emoji:newEmoji, points:newPts, memberId:effectiveNewMember, claimed:false }])
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'var(--bg)' }}>
      ...
      <div style={{ padding:'20px 24px 12px', background:'var(--surface)', borderBottom:'1px solid var(--border)' }}>
        ...
        <div style={{ display:'flex', gap:10, marginBottom:12, flexWrap:'wrap' }}>
          {members.map(m => (
            <div key={m.id} style={{ padding:'8px 14px', borderRadius:12, background:m.bgColor, border:`1px solid ${m.barColor}40`, display:'flex', alignItems:'center', gap:8 }}>
              <MemberAvatar member={m} size={28}/>
              <div>
                <p style={{ fontSize:11, fontWeight:600, color:m.textColor, fontFamily:'Inter' }}>{m.name}</p>
                <p style={{ fontSize:16, fontWeight:800, color:m.textColor, fontFamily:'Inter', display:'flex', alignItems:'center', gap:3 }}>
                  <Star size={12}/> {points[m.id] ?? 0}
                </p>
              </div>
            </div>
          ))}
          {members.length === 0 && (
            <p style={{ fontSize:13, color:'var(--text-3)', fontFamily:'Inter' }}>Add members in Settings to track rewards</p>
          )}
        </div>
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          <button onClick={()=>setFilter(null)}
            style={{ padding:'5px 14px', borderRadius:20, border:`1.5px solid ${!filter?'var(--blue)':'var(--border)'}`, background:!filter?'var(--blue-bg)':'var(--surface)', color:!filter?'var(--blue)':'var(--text-2)', fontSize:12, fontWeight:600, fontFamily:'Inter', cursor:'pointer' }}>
            All
          </button>
          {members.map(m => (
        ...
        {showAdd && (
          <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}}
            ...
            <div style={{ padding:'14px 24px', display:'flex', gap:10, flexWrap:'wrap', alignItems:'flex-end' }}>
              ...
              <input type="number" value={newPts} onChange={e=>setNewPts(Number(e.target.value))} min={10} max={9999}
              ...
              <select value={effectiveNewMember} onChange={e=>setNewMember(e.target.value)}
              ...
              <motion.button whileTap={{scale:0.95}} onClick={add}
                style={{ padding:'9px 20px', borderRadius:10, background:'var(--blue)', color:'#fff', border:'none', fontSize:13, fontWeight:700, fontFamily:'Inter', cursor:'pointer' }}>
                Add
              </motion.button>
            </div>
          </motion.div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px 24px' }}>
        {visible.length === 0 ? (
          <div style={{ textAlign:'center', paddingTop:60, color:'var(--text-3)' }}>
            ...
            <p style={{ fontWeight:700, fontFamily:'Inter', fontSize:16, color:'var(--text-2)' }}>No rewards yet</p>
            <p style={{ fontSize:13, color:'var(--text-3)', fontFamily:'Inter', marginTop:6 }}>Click "+ Add Reward" to create your first one</p>
          </div>
        ) : (
              ...
              return (
                <motion.div key={r.id} layout initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}
                  ...
                  <button onClick={()=>remove(r.id)}
                    style={{ position:'absolute', top:10, right:10, width:24, height:24, borderRadius:6, border:'none', background:'transparent', cursor:'pointer', color:'var(--text-3)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <Trash2 size={13}/>
                  </button>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:14 }}>
                    <span style={{ fontSize:12, fontWeight:700, color:m.textColor, background:m.bgColor, padding:'3px 10px', borderRadius:20 }}>{m.emoji} {m.name}</span>
                    <span style={{ fontSize:13, fontWeight:700, color:'#F59E0B' }}>⭐{r.points}</span>
                  </div>
                  <motion.button whileTap={{scale:0.95}} onClick={()=>claim(r)} disabled={!canClaim}
